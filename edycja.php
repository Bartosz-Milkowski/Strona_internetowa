...
if( $ile > 0) {
    ...
    while($r = $wynik->fetch_assoc()) {
        echo "<tr>";
        echo '<td><a href="panel.php?subpage=doedycji&id='.$r['ID'].'&Imie='.$r['Imie'].'&Nazwisko='.$r['Nazwisko'].'&Plec='.$r['Plec'].'&Nazwisko_pan='.$r['Naz.pan'].'&Email='.$r['Email'].'&Kod='.$r['Kod'].'">Edycja</a></td>';
        echo "<td>".$r['ID']."</td>";
        echo "<td>".$r['Imie']."</td>";
...
<?php
    if($strona >= 1)
    {
      echo '<a href="http://mb41449.cba.pl/panel.php?subpage=edycja&strona=1"> <-  </a>  ';  
    }
    else
    echo ("  <-");  
    ...
    for( $i; $i <= $wszystko; $i++)
    {
    echo '<a href="http://mb41449.cba.pl/panel.php?subpage=edycja&strona='.$i.'">'.$i.'</a>  ';
    }
    
    if($strona < ($wszystko-1))
    {
      echo '<a href="http://mb41449.cba.pl/panel.php?subpage=edycja&strona='.$wszystko.'"> -> </a>  ';  
    }
    else
    echo '->';  
}
